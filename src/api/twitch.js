...
export async function fetchClipsCards(
  ...
  try {
    ...
    if (cursor) {
      ...
      console.log(`📍 Full request variables:`, JSON.stringify(variables, null, 2));
    }
    ...
    console.log(`📄 pageInfo:`, JSON.stringify(clipsConnection?.pageInfo, null, 2));
    ...
    if (edges.length > 0) {
      ...
      if (edgesWithCursors.length > 0) {
        console.log(`🔍 Edges with cursors:`, 
          edgesWithCursors.map(({ cursor, idx }) => {
            try {
      ...
      for (let i = edges.length - 1; i >= 0; i--) {
        if (edges[i].cursor) {
          ...
          try {
            ...
            console.log(`✅ Next cursor from edge[${i}]: ${endCursor.substring(0, 10)}... (offset: ${decoded})`);
          } catch (e) {
            console.log(`✅ Next cursor from edge[${i}]: ${endCursor.substring(0, 10)}...`);
          }
          break;
        }
      }
    ...
    if (!endCursor) {
      console.log(`⚠️ No cursor found in edges (pagination may not be available)`);
    }
    
    console.log(`📄 hasNextPage: ${pageInfo.hasNextPage}`);

    console.log(`ClipsCards fetch successful: ${clips.length} clips, hasNextPage: ${pageInfo.hasNextPage}`);
...
export async function fetchMultipleCriteriaClips(channelName, days = 900) {
  console.log(`Fetching clips with multiple filters using ClipsCards for ${channelName} (${days} days)...`);
  ...
  try {
    ...
    const fetchPromises = uniqueFilters.map(filter =>
      fetchClipsCards(channelName, 100, filter)
        ...
        .catch(error => {
          ...
          return { clips: [], filter, success: false, hasNextPage: false, endCursor: null };
        })
    );
    ...
    results.forEach(({ clips, filter, success, hasNextPage, endCursor }) => {
      ...
      if (filter === primaryFilter && success) {
        primaryFilterPagination = { hasNextPage, endCursor, filter: primaryFilter };
      }
    ...
    console.log(`Combined ${allClips.length} unique clips from ${uniqueFilters.length} ClipsCards fetches`);
    ...
    if (allClips.length < 20) {
      ...
      try {
        ...
        while (pageCount < maxPages) {
          const fallbackResult = await fetchClipsCards(channelName, 100, "ALL_TIME", cursor);
          const newClips = fallbackResult.clips.filter(clip => !currentSeenIds.has(clip.id));
        ...
        console.log(`Added ${newClips?.length || 0} additional clips from paginated fallback`);
      } catch (fallbackError) {
...
export async function fetchDiverseClips(
  ...
  try {
    ...
    console.log(`Using filter: ${filter}${cursor ? ` with initial cursor: ${cursor.substring(0, 10)}...` : ''}`);

    // Fetch multiple pages to get beyond just the top clips
    while (allClips.length < totalClips && requestCount < maxRequests) {
      console.log(`Request ${requestCount + 1}: Using cursor: ${cursor ? cursor.substring(0, 10) + '...' : 'null'}`);
      
      const result = await fetchClipsCards(channelName, 100, filter, cursor);

      if (result.clips.length === 0) {
        console.log(`No clips returned on request ${requestCount + 1}, breaking`);
        break;
      }
      ...
      console.log(`Request ${requestCount} completed: ${result.clips.length} clips, hasNextPage: ${result.hasNextPage}, newCursor: ${result.endCursor ? result.endCursor.substring(0, 10) + '...' : 'null'}`);

      if (!result.hasNextPage || !result.endCursor) {
        console.log(`No more pages available for ${filter} (hasNextPage: ${result.hasNextPage}, endCursor: ${result.endCursor ? 'exists' : 'null'})`);
        break;
      }
      ...
      if (previousCursor && cursor === previousCursor) {
        console.error(`🚨 CURSOR DID NOT CHANGE! Previous: ${previousCursor.substring(0, 10)}..., New: ${cursor.substring(0, 10)}...`);
    ...
    if (allClips.length < totalClips && filter !== "ALL_TIME" && !startCursor) {
      ...
      try {
        ...
        console.log(`Added ${newClips?.length || 0} additional unique clips from ALL_TIME`);
      } catch (error) {
